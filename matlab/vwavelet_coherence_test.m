...
% for nn=1:3
%     subplot(4,2,(nn*2)-1)
%     xwt(cell2mat(xwt_data_left_reference(nn)),cell2mat(xwt_data_left(nn)),'S0',min_scale_left,'maxscale',max_scale_left,'Pad',1,'ArrowDensity',Arrow_Density,'ArrowSize',arrow_size,'ArrowHeadSize',Arrow_Head_Size)
...
% for nn=1:4
%     subplot(4,2,nn*2)
%     xwt(cell2mat(xwt_data_right_reference(nn)),cell2mat(xwt_data_right(nn)),'S0',min_scale_right,'maxscale',max_scale_right,'Pad',1,'ArrowDensity',Arrow_Density,'ArrowSize',arrow_size,'ArrowHeadSize',Arrow_Head_Size)
...
wtc_data_left={vieira,steinhilber09,steinhilber12};
wtc_data_left_reference={roth_10,roth_5,roth_22};
wtc_names_left={'WTC between Roth and Vieira','WTC between Roth and Steinhilber (2009)','WTC between Roth and Steinhilber (2012)'};

for nn=1:3
    subplot(4,2,(nn*2)-1)
    wtc(cell2mat(wtc_data_left_reference(nn)),cell2mat(wtc_data_left(nn)),'S0',min_scale_left,'maxscale',max_scale_left,'Pad',1,'ArrowDensity',Arrow_Density,'ArrowSize',arrow_size,'ArrowHeadSize',Arrow_Head_Size)
    title(wtc_names_left(nn),'fontsize',font_size);
...
wtc_data_right={sbf,db,mea,vk};
wtc_data_right_reference={roth,roth,roth,roth};
wtc_names_right={'WTC between Roth and SBF','WTC between Roth and DB','WTC between Roth and MEA','WTC between Roth and VK'};

for nn=1:4
    subplot(4,2,nn*2)
    wtc(cell2mat(wtc_data_right_reference(nn)),cell2mat(wtc_data_right(nn)),'S0',min_scale_right,'maxscale',max_scale_right,'Pad',1,'ArrowDensity',Arrow_Density,'ArrowSize',arrow_size,'ArrowHeadSize',Arrow_Head_Size)
    title(wtc_names_right(nn),'fontsize',font_size);
    set(gca,'fontsize',font_size);
    ylabel('Period','fontsize',font_size)
    h4=line([800 2000],[log2(period_1) log2(period_1)],[0 0]);
    h5=line([800 2000],[log2(period_2) log2(period_2)],[0 0]); 
...
%export_fig('../../plots/wavelets/wtc','-png','-opengl','-r200')
%close;
